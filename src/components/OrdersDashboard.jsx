  ...
  return (
    <tr style={{ borderBottom: '1px solid var(--border-subtle)', transition: 'background 0.15s' }}
      ...
      <td style={{ padding: '15px 20px' }}>
        {order.amount > 15000 || order.status === 'Processing' ? (
          <span style={{ background: 'rgba(239, 68, 68, 0.1)', color: '#ef4444', padding: '2px 8px', borderRadius: 4, fontSize: 10, fontWeight: 700, textTransform: 'uppercase' }}>High</span>
        ) : (
          <span style={{ color: 'var(--text-muted)', fontSize: 10, fontWeight: 500, textTransform: 'uppercase' }}>Normal</span>
        )}
      </td>
    ...
    <tr>
      <td colSpan={6}>
        <div style={{ padding: '80px 32px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 20, fontWeight: 700, color: 'var(--text-primary)' }}>No orders found</div>
          <p style={{ color: 'var(--text-secondary)', maxWidth: 320, lineHeight: 1.6, fontSize: 14 }}>
            It looks like there aren't any orders to display right now. This could be because no orders have been placed yet or filters are too restrictive.
          </p>
          <button 
    ...
    <tr>
      <td colSpan={6}>
        <div style={{ padding: '80px 32px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 20, fontWeight: 700, color: 'var(--text-primary)' }}>Unable to load orders</div>
          <div style={{ 
            background: 'var(--surface-2)', padding: '12px 20px', borderRadius: 8,
            color: '#ef4444', maxWidth: 400, fontSize: 13, fontFamily: 'var(--mono)',
            border: '1px solid rgba(239, 68, 68, 0.2)', lineHeight: 1.5
          }}>
            {message || 'An unexpected error occurred while fetching the order data.'}
          </div>
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 32px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 32 }}>
        {[
          { label: 'Total Orders',     value: loading ? '—' : orders.length,                        icon: '📦', color: 'var(--accent)'  },
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)' }}>
                {['Order ID', 'Customer', 'Priority', 'Amount', 'Status', 'Date'].map(h => (
